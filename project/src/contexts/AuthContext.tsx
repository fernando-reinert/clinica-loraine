...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const signUp = async (email: string, password: string, name: string, profession: string) => {
    try {
      ...
      // Salvar a profissão na tabela 'professionals'
      const { error: insertError } = await supabase
        .from('professionals') // Assumindo que você tem a tabela 'professionals'
        .insert([
          {
            user_id: data.user?.id, // ID do usuário recém-criado
            name,
            profession, // Salvando a profissão
          },
        ]);
