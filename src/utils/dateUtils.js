 ...
 */
export const convertToSupabaseFormat = (dateString) => {
    if (!dateString) {
 ...
 */
export const convertToBrazilianFormat = (isoString) => {
    if (!isoString)
 ...
 */
export const brazilianStringToDate = (dateString) => {
    if (!dateString)
        return null;
    try {
 ...
 */
export const isFutureDate = (isoString) => {
    if (!isoString)
        return false;
    try {
        const date = new Date(isoString);
 ...
 */
export const getMinutesDifference = (startISO, endISO) => {
    if (!startISO || !endISO)
        return null;
    try {
 ...
 */
export const addMinutesToDate = (isoString, minutes) => {
    if (!isoString || isNaN(minutes))
        return null;
    try {
        const date = new Date(isoString);
        if (isNaN(date.getTime()))
 ...
 */
export const formatFullExtendedDate = (isoString) => {
    if (!isoString)
        return "Data não informada";
    try {
        const date = new Date(isoString);
