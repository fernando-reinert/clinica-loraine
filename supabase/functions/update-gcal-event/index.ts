// feat(calendar): update-gcal-event — patch Google Calendar event (remarca)
// Body: { eventId, patientName?, start?, end?, notes? }. Retorna { ok, eventId, htmlLink }.
...
function toRfc3339(isoOrLocal: string): string {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { eventId, patientName, start, end, notes } = body ?? {};

    if (!eventId || typeof eventId !== "string" || eventId.trim() === "") {
      console.warn("[gcal] update: eventId obrigatório");
      return new Response(
    ...
    if (!clientEmail || !privateKey || !calendarId) {
      console.error("[gcal] update: secrets não configurados");
      return new Response(
    ...
    if (patientName != null) patchBody.summary = `Consulta - ${latin1Safe(patientName)}`;
    if (start != null && end != null) {
      patchBody.start = { dateTime: toRfc3339(start), timeZone: "America/Sao_Paulo" };
      patchBody.end = { dateTime: toRfc3339(end), timeZone: "America/Sao_Paulo" };
    }
    if (notes != null) patchBody.description = latin1Safe(String(notes));

    if (Object.keys(patchBody).length === 0) {
      return new Response(
        JSON.stringify({ ok: true, eventId, htmlLink: null }),
    ...
    if (!res.ok) {
      ...
      if (res.status === 404) {
        console.warn("[gcal] update: evento não encontrado", eventId);
        return new Response(
          JSON.stringify({ ok: false, error: "Evento não encontrado no Google Calendar" }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      console.error("[gcal] update: API error", res.status, errText.slice(0, 300));
      return new Response(
        JSON.stringify({ ok: false, error: "Falha ao atualizar evento", details: errText.slice(0, 200) }),
    ...
    console.log("[gcal] update: ok", eventId);
    return new Response(
      JSON.stringify({ ok: true, eventId, htmlLink }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (e) {
    console.error("[gcal] update: exception", e?.message ?? String(e));
    return new Response(
