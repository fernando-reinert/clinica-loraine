...
-- Objetivo: Criar tabela para cadastro público de pacientes via link
-- ============================================
...
BEGIN
  ...
  -- Atualizar respostas (usar payload)
...
-- ============================================
-- 9. PERMISSÕES: Permitir execução anônima dos RPCs
-- ============================================
-- Permitir que usuários anônimos executem os RPCs (necessário para formulários públicos)
GRANT EXECUTE ON FUNCTION create_patient_signup_form(int) TO anon;
GRANT EXECUTE ON FUNCTION get_signup_form_by_token(uuid) TO anon;
GRANT EXECUTE ON FUNCTION update_signup_form_answers(uuid, jsonb) TO anon;
GRANT EXECUTE ON FUNCTION complete_patient_signup_form(uuid, jsonb) TO anon;
GRANT EXECUTE ON FUNCTION create_patient_anamnese_form(uuid, int) TO anon;
...
COMMENT ON COLUMN patient_signup_forms.share_token IS 'Token UUID único para acesso público ao formulário';
COMMENT ON COLUMN patient_signup_forms.status IS 'Status: sent (enviado), completed (concluído) ou expired (expirado)';
COMMENT ON COLUMN patient_signup_forms.payload IS 'Dados do formulário em JSONB (opcional, pode salvar o que o paciente digitou)';
COMMENT ON COLUMN patient_signup_forms.share_expires_at IS 'Data de expiração do link (30 dias padrão)';
